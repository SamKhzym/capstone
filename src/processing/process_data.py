...
class SerialData:
    ...
    def plotMetrics(self):
        ...
        ax[0].plot(s.times, s.dutyCycles1[0:length], label="Duty Cycle 1 (%)")
        ax[0].axhline(y = 65, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[0].axhline(y = 60, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[0].legend()

        ax[1].plot(s.times, s.dutyCycles2[0:length], label="Duty Cycle 2 (%)")
        ax[1].axhline(y = 50, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[1].axhline(y = 40, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[1].legend()

        ax[2].plot(s.times, s.dynoPwm1[0:length], label="Duty Cycle 1 (%)")
        ax[2].axhline(y = 55, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[2].axhline(y = 50, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[2].legend()

        ax[3].plot(s.times, s.dynoPwm2[0:length], label="Duty Cycle 2 (%)")
        ax[3].axhline(y = 49, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[3].axhline(y = 46, color = 'r', linestyle = '-', label='Threshold (%)') 
        ax[3].legend()

        ax[4].plot(s.times, s.pwmSpeeds[0:length], label="PWM Output Commands (0-255)")
        ax[4].legend()

        ax[5].plot(s.times, s.wheelSpeeds1[0:length], label="Linear Wheel Speed W1 (m/s)", color='red')
        ax[5].plot(s.times, s.wheelSpeeds2[0:length], label="Linear Wheel Speed W2 (m/s)", color='blue')
        ax[5].plot(s.times, s.dynoWheelSpeeds1[0:length], label="Linear Wheel Speed T1 (m/s)", color='green')
        ax[5].plot(s.times, s.dynoWheelSpeeds2[0:length], label="Linear Wheel Speed T2 (m/s)", color='purple')
        ax[5].plot(s.times, s.vehSpeeds[0:length], label="Linear Veh Speed (m/s)", color='black', linewidth=3)
