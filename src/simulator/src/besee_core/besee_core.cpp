...
BeseeCore::BeseeCore(EgoVehicle& ego,
                     vector<shared_ptr<PathFollowingActor>>& actors,
                     LaneDefinition& lanes,
                     int updateRateHz): actors(actors), lanes(lanes), updateRateHz(updateRateHz)
{
    this->ego = ego;
}
...
BeseeCore::BeseeCore(string scenarioName, int updateRateHz)
{
    ...
    for (auto segment : roadsNode["lane_segments"]) {
        ...
        for (auto map : segment) {
            ...
            if (laneType == "straight") {
                laneSegments.push_back(StraightLaneSegment(laneProperties["length"].as<double>()));
            }

            // if dict key is "arc", parse ArcLaneSegment
            else if (laneType == "arc") {
                laneSegments.push_back(ArcLaneSegment(laneProperties["radius"].as<double>(),
    ...
    LaneDefinition lanes = LaneDefinition(laneSegments, roadsNode["num_lanes"].as<int>(),
    ...
    for (auto actor : actorsNode) {
        ...
        for (auto map : actor) {
            ...
            shared_ptr<DriveCycle> driveCycle =
                vehProperties["speed_profile"] ? shared_ptr<DriveCycle>(parseDriveCycleCsv(
                                                     vehProperties["speed_profile"].as<string>()))
                                               : nullptr;

            actors.push_back(make_shared<PathFollowingActor>(
                vehName, lanes.getLaneWaypoints(vehProperties["lane_id"].as<int>()),
                ...
                allActors[vehModel]["width"].as<double>(), driveCycle));
        }
    }

    // construct ego vehicle
    string egoMake = egoNode["make"].as<string>();
    EgoVehicle ego("ego", 0, 0, 0, allActors[egoMake]["length"].as<double>(),
    ...
    if (controllerName == "cacc") {
        ...
        cout << "NOTE: CONTROLLER TYPE " << controllerName
             << " NOT SUPPORTED YET. DEFAULTING TO CACC." << "\n";
        this->simController = new SimCaccController(this, 27.777);
    }
...
void BeseeCore::update(double dt)
{
    ...
    for (auto actor : this->actors) {
        actor->update(1.0f / updateRateHz);
    }
}
...
void BeseeCore::runSim(double endTime, double timeRatio)
{
    ...
    for (int ts = 0; ts < (endTime * this->updateRateHz); ts++) {
        ...
        // printing stuff
        std::cout << "=========== TS = " << (double)ts / (double)this->updateRateHz
                  << "sec ==========" << "\n";
...
unordered_map<string, VectorXd>& BeseeCore::getActorStatesInEgoFrame()
{
    ...
    for (shared_ptr<PathFollowingActor> actor : this->actors) {
        ...
        // get rotation matrix and rotate relative state by delta heading to get proper x and y
        // values
        ...
        actorStates->insert(pair(actor->name, relActorState));
    }
...
void BeseeCore::setCavLateralCommands(double steeringTorqueReq_Nm)
{
    this->ego.cavSteeringTrqReq_Nm = steeringTorqueReq_Nm;
}
...
void BeseeCore::setSteeringWheelPosition(double steeringWheelPosition_rad)
{
    this->ego.steeringWheelAngle_rad = steeringWheelPosition_rad;
}

void BeseeCore::setCavEnable(bool cavEnable)
{
    this->ego.cavEnable = cavEnable;
}