#include "utils/common/functions.hpp"
...
StraightLaneSegment::StraightLaneSegment(double length_m, double resolution_m):
    length_m(length_m), resolution_m(resolution_m)
{

    // for straight line, x values are just equally spaced from 0 to length, y values and headings
    // are all 0
...
ArcLaneSegment::ArcLaneSegment(double radius_m,
                               double rotation_pct,
                               bool clockwise,
                               double resolution_m):
    radius_m(radius_m), rotation_pct(rotation_pct), clockwise(clockwise), resolution_m(resolution_m)
{
...
SineLaneSegment::SineLaneSegment(
    double period_m, double amplitude_m, double offset_m, double numCycles, double resolution_m):
    period_m(period_m), amplitude_m(amplitude_m), offset_m(offset_m), resolution_m(resolution_m)
{

    // for sine lane segments, x values are just 0 to period * numcycles,
    // y values are a * sin(2pi/T (x - d)) - y0, headings are arctan(dy/dx)
    ...
    VectorXd yVals =
        this->amplitude_m * sin(2 * M_PI / this->period_m * (xVals.array() - this->offset_m));
    yVals.array() -= yVals(0);
    auto headings = atan(2 * M_PI * this->amplitude_m / this->period_m *
                         cos(2 * M_PI / this->period_m * (xVals.array() - this->offset_m)));
...
LaneDefinition::LaneDefinition(vector<LaneSegment>& segments, int numLanes, double laneWidth_m):
    numLanes(numLanes), laneWidth_m(laneWidth_m)
{
    ...
    this->laneBoundaryMap.insert(
        make_pair(0, *Path2D(basePath).transformPathAlongHeading(this->laneWidth_m / 2, false)));

    // create numLanes new lanes/lane boundaries, offset them by the approproate amount, insert them
    // into maps
    for (int i = 0; i < this->numLanes; i++) {

        this->laneMap.insert(
            make_pair(i, *Path2D(basePath).transformPathAlongHeading(i * this->laneWidth_m)));
        this->laneBoundaryMap.insert(
            make_pair(i + 1, *Path2D(basePath).transformPathAlongHeading(i * this->laneWidth_m +
                                                                         this->laneWidth_m / 2)));
    }
}
