...
#include "utils/common/matrix_utils.h"
#include <Eigen/Dense>
#include <algorithm>
#include <cstddef>
...
void Path2D::updateDistance()
{
    ...
    VectorXd distances =
        sqrt((x_deltas.array() * x_deltas.array() + y_deltas.array() * y_deltas.array()).array());

    // set class attribute
    this->distances = cumulativeSum(distances);
}
...
Matrix13d Path2D::distanceToWaypoint(double distance_m)
{
    ...
    Matrix13d waypoint({{interpolate1d(this->distances, x_vals, distance_m),
...
void Path2D::transformPath(double x_m, double y_m, double heading_rad)
{
    ...
    if (x_m != 0) {
        (this->waypoints).col(0).array() += x_m;
    }

    // translate path by y if y is nonzero
    if (y_m != 0) {
        (this->waypoints).col(1).array() += y_m;
    }
...
Path2D *Path2D::transformPathAlongHeading(double offset_m, bool left)
{
    ...
    // get waypoint headings of centerlane and construct vector of specified length in that
    // direction
...
VectorXd Path2D::getXVals()
{
    return (VectorXd)((this->waypoints).col(0));
}
VectorXd Path2D::getYVals()
{
    return (VectorXd)((this->waypoints).col(1));
}
VectorXd Path2D::getHeadings()
{
    return (VectorXd)((this->waypoints).col(2));
}
