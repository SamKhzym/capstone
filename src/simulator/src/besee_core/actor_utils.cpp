...
#include "utils/csv/csv_utils.hpp"
#include <cstddef>
#include <memory>
#include <vector>
...
double DriveCycle::getSpeed(double time)
{
    Eigen::VectorXd times =
        Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(this->times.data(), this->times.size());
    Eigen::VectorXd speeds = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(this->speeds_mps.data(),
                                                                           this->speeds_mps.size());
    return interpolate1d(times, speeds, time);
}

PathFollowingActor::PathFollowingActor(string name,
                                       Path2D path,
                                       double speed_mps,
                                       double initDist_m,
                                       double length_m,
    ...
    path(path), speed_mps(speed_mps), distAlongPath(initDist_m)
{
    this->name = name;
    this->length_m = length_m;
    this->width_m = width_m;
...
{
    this->time += dt;
    if (this->driveCycle != nullptr) {
        this->speed_mps = driveCycle->getSpeed(this->time);
    }
...
EgoVehicle::EgoVehicle(
    string name, double init_x, double init_y, double init_heading, double length_m, double width_m)
{
    this->name = name;
    this->length_m = length_m;
    this->width_m = width_m;
...
void EgoVehicle::update(double dt)
{
    ...
    while (this->timestep <= endTimestep) {
        model.step0();
        if (this->timestep % (int)(BASE_SAMPLE_RATE_HZ / TASK_2_SAMPLE_RATE_HZ) == 0) {
            this->model.step2();
        }
        if (this->timestep % (int)(BASE_SAMPLE_RATE_HZ / TASK_3_SAMPLE_RATE_HZ) == 0) {
            this->model.step3();
        }
        if (this->timestep % (int)(BASE_SAMPLE_RATE_HZ / TASK_4_SAMPLE_RATE_HZ) == 0) {
            this->model.step4();
        }
        this->timestep++;
    }
