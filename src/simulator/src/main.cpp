...
scenarioName: a scenario name defined in scenarios.yaml
[optional, default value open_straight_road]

endTime: how long to run the sim in seconds
[optional, default value 100]

outputCsvName: name of logging csv to output
[optional, default value loggers_<scenarioName>.csv]
...
int main(int argc, char **argv)
{
    ...
    // if user passed in a location to save loggers csv, set that. Otherwise, set location to
    // "loggers_<scenarioName>.csv"
    string csvSaveLocation = "loggers_" + configScenarioname;
    if (argc >= 4) {
        csvSaveLocation = argv[3];
    }
    csvSaveLocation += ".csv";
    ...
    // run besee!
    std::cout << "Running scenario " << configScenarioname << " for " << endTime << " seconds!"
              << std::endl;
